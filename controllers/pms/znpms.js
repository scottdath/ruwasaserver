...
export const getznData =(req,res)=>{
    data.getConnection((err,connection)=>{
        if (!err){
    ...
    data.getConnection((err,connection)=>{
        if (!err){
            connection.query('SELECT * FROM znmessages',(err,rows)=>{
                ...
                const q = "UPDATE znmessages SET metertype=?,meterid=?,username=?,region=?,location=?,regionId=? WHERE meterno = ?"
...
export const deleteMeter =(req,res)=>{
    data.getConnection((err,connection)=>{
        if (!err){
            ...
            const q  = "DELETE FROM znmessages WHERE `meterno`= ?"
